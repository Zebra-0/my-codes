...
import time
# Notas da versão:
# adicionando log de eventos
# especificando nome do arquivo de acordo com título pego em webscraper
# ajustada abertura única da janela de configurações
# bug de fixar no topo não funcionar após abrir o menu de configurações pela segunda vez
# removido alguns prints por logs
# copiar agora busca dados de smd

# P/ outras versões:
# continuar a adicionar log de eventos.
# botão para interromper download
# adicionar report de downloads
# adiconar notificações do windows.
...
class Programa:
    ...
    def baixar_video(self):
        ...
        if not validado:
            ...
            self.canvas.itemconfig(self.notificacao_topo, text=f"{str(title)}...")
            self.canvas.itemconfig(self.notificacao_meio, text=f"Baixando: {str(title)}...")
            self.link_data(self.link_input.get()).streams.get_highest_resolution().download(output_path=self.pasta_usuario, filename=f"{title}.mp4")
            logger.info(f"arquivo salvo {title}, em: {self.pasta_usuario}")
    ...
    def baixar_audio_playlist(self):
        ...
        for audio in audio_playlist.videos:
            ...
            if not validado:
                ...
                self.link_data(audio.watch_url).streams.get_audio_only().download(output_path=self.pasta_usuario, filename=f"{title}.mp4")
                logger.info(f"arquivo salvo {title}, em: {self.pasta_usuario}")
    ...
    def baixar_video_playlist(self):
        ...
        for video in video_playlist.videos:
            ...
            if not validado:
                ...
                self.link_data(video.watch_url).streams.get_highest_resolution().download(output_path=self.pasta_usuario, filename=f"{title}.mp4")
                logger.info(f"arquivo salvo {title}, em: {self.pasta_usuario}")
    ...
    def func_btt_baixar(self, *args):
        ...
        if not self.midia:
            logger.error("Link inválido, verifique se o link possui youtube.com/ ou youtu.be/")
            pass
    ...
    def func_btt_config(self, *args):
        if not self.janela_config or not self.janela_config.winfo_exists():  # controla abertura única
            ...
            self.fixar_janela_button = Button(self.janela_config, text=f"{self.fixar_on_top}", command=self.fixar_janela)
...
if __name__ == "__main__":
    root = Tk()
    root.title("Simple Music Downloader - SMD 0.5.2 by suportedoluiz")
