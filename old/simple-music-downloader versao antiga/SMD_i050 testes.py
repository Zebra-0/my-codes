...
# assegurar_arquivo, read_json_config, new_pc_json agora estão em smd
# logica do conversor mp4 e configurações de pasta movidos para smd
# resolvido bug que a janela de config travava ao converter
# adicionado verificador de duplicados.
# _ficou faltando: mover lógica do link e porcentagem. ajustar melhor a %
...
#        self.iniciar_temporizador_inatividade()
    ...
    def update_progresso(self, stream, chunk, bytes_faltando):
        ...
        threading.Thread(target=lambda: root.update_idletasks()).start()
        if self.porcent == 100:
    ...
    def baixar_audio(self):
        ...
        if not validado:
            ...
            self.link_data(self.link_input.get()).streams.get_audio_only().download(output_path=self.pasta_usuario,filename=f"{title}.mp4")
...
if __name__ == "__main__":
    root = Tk()
    root.title("Simple Music Downloader - SMD 0.5.0")
