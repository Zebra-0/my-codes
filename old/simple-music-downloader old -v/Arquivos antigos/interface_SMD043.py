...
# Notas da versão:
# titulo do vídeo ou playlist a ser baixado no topo
# criada pasta interna para separar as músicas.
# adicionado um conversor para mp4 para mp3
# adicionada pasta do usuário
# arquivo json adicionado para salvar preferências do usuário
# --
# lista de download
# adicionar a lista pela música tocando no navegador
...
class Programa:
    def __init__(self, root):
        ...
        self.notificacoes = self.canvas.create_text(97, 89, text="",
        ...
        self.titulo_midia = self.canvas.create_text(300, 20, text="",
                                                    font=("Monserrat", 10), anchor="nw")
        ...
        self.configuracoes = self.read_json_config()
    ...
    def titulo_cabecalho(self):
        self.canvas.itemconfig(self.titulo_midia, text=f"{self.link_data(self.link()).title}")

    def baixar_audio(self):
        print("func baixar audio iniciada")
        self.titulo_cabecalho()
        self.canvas.itemconfig(self.notificacoes, text=f"Baixando: {self.link_data(self.link()).title}...")
        self.link_data(self.link()).streams.get_audio_only().download(self.subpasta)
    ...
    def func_btt_baixar(self, *args):
        ...
        link = self.link()
        tipo = smd.validador_de_link(str(self.link()))
        if self.midia == None:
            pass
    ...
    def func_btt_pasta(self, *args):
        ...
        if self.pasta_caminho:
            ...
            self.pasta_caminho = self.pasta_caminho.replace("/","\\")
            self.configuracoes["PastaUsuarioPadrao"] = self.pasta_caminho
    ...
    def conversor_mp4(self):
        ...
        dir = "D:\\Users\\remote\\PycharmProjects\\Goals\\audio-video\\"
        arquivos = os.listdir(dir)
        arquivos_mp4 = []
        try:
            for suspeito in arquivos:
                if ".mp4" in suspeito:
                    arquivos_mp4.append(dir+suspeito)
            if self.autoconvert:
                for mp4 in arquivos_mp4:
                    destino_mp3 = mp4.replace(".mp4",".mp3")
                    if not destino_mp3:
                        threading.Thread(target=smd.mp4_mp3(mp4, destino_mp3))
                    else:
...
if __name__ == "__main__":
    root = Tk()
    root.title("Simple Music Downloader - SMD 0.4.3")
    app = Programa(root)
    root.mainloop()
