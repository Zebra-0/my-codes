...
command_output = subprocess.run(["netsh", "wlan", "show", "profiles"], stdout=)
...
profile_names = (re.findall("Todos os Perfis de Usuários:      (.*)\r", str(command_output)))
print(profile_names)
...
if len(profile_names) != 0:
    ...
    for name in profile_names:
        ...
        profile_info = subprocess.run(["netsh", "wlan", "show", "profile",  f'"{name}"'])
        #    We use the regular expression to only look for the absent cases so we can ignore them.
        if re.search("Conteúdo da Chave:           : ausente", str(profile_info)):
            continue
        else:
            ...
            password = re.search("Conteúdo da Chave            : (.*)\r", profile_info_pass)
...
for x in range(len(wifi_list)):
    print(wifi_list[x], "está lendo isso")
