...
class JanelaPrincipal:
    def __init__(self, janela):
        ...
        self.unidades = ['Centro', 'Tijuca (Sans Peña)', 'Tijuca (Maj Ávila)', 'Botafogo', 'Copacabana', 'Ramos', 'Campo Grande', 'Cascadura', 'Caxias', 'Grande Rio', ' Jacarepaguá', 'Catete', 'Ilha do Governador', 'Madureira', 'Norte Shopping', 'Santa Cruz', 'São Gonçalo']
        self.forget = None
        ...
        ''' Posicionando botões fixos'''
        self.salvartxt_botao.place(x=self.confi["botoes"]["salvar"]["x"], y=self.confi["botoes"]["salvar"]["y"])

        self.escolherpasta_botao.place(x=self.confi["botoes"]["escolher-pasta"]["x"],
                                       y=self.confi["botoes"]["escolher-pasta"]["y"])
        self.limpar_botao.place(x=self.confi["botoes"]["limpar"]["x"], y=self.confi["botoes"]["limpar"]["y"])
        self.copiar_botao.place(x=self.confi["botoes"]["copiar"]["x"], y=self.confi["botoes"]["copiar"]["y"])
        ...
        self.abrirfeegow2_botao = ttk.Button(self.frame_dados, text="Feegow backup", command=lambda: abrir_sites("feegow2"))
    ...
    def load_layout(self, *args: object):
        ...
        if self.solicitacao.get() == "Padrão":
            ...
            self.tel_entry.place(x=self.confi["tel"]["layout"][layout]["xentry"],
                                 y=self.confi["tel"]["layout"][layout]["y"] + self.espacamento, width=195, height=28)
            self.email_entry.place(x=self.confi["email"]["layout"][layout]["xentry"],
                                   y=self.confi["email"]["layout"][layout]["y"] + (self.espacamento * 2), width=280,
                                   height=28)
