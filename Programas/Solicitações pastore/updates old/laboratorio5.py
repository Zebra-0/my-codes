...
class JanelaPrincipal:
    ...
    def is_valid_date(self, date_string):
        if not all(c.isdigit() or c == '/' for c in date_string):
            return False
        # mais verificações podem ser adicionadas aqui, se necessário
        return True

    def validate_dates(self, dates):
        for date in dates:
            if not self.is_valid_date(date_string=date):
                messagebox.showerror("Erro",
                                     f"Data de exame inválida: {date}. Por favor insira uma data no formato DD/MM/AAAA.")
                return False
        return True

    def format_motivo(self, motivo, data_exame, data_exame2, data_exame3, data_exame4, procedimento):
    ...
    def custom_text_for(self, text, tratado=True):
        if text == "Extensao Retorno":
            motivo = "Paciente entrou em contato solicitando\na extensão de retorno, o mesmo informou\n" \
                     "que realizou o procedimento de \n\nno dia [00/00/0000], e vai ficar pronto\n" \
            ...
            if tratado:
                ...
                procedimento = str(self.procedimento_entry.get())
                print("procedimento" + str(procedimento))
                validate = [data_exame, data_exame2, data_exame3, data_exame4]
                if all(validate):
                    if self.validate_dates(validate):
                        return self.format_motivo(motivo, data_exame, data_exame2, data_exame3, data_exame4,
                                                  procedimento)
                else:
        ...
        if carregar_layout == True:
            if layout == "Padrão":
                ...
                motivo = f"Paciente entrou em contato solicitando\na extensão de retorno," \
                         ...
                         f"encerrou no dia [--/--/----], sendo\nassim, podemos agendar o paciente para a " \
    ...
    def copy_dados(self, msg=True, copy=True, to_whatsapp=False):
        ...
        if solicitacao == "Padrão":
            ...
            dados += f'\nSolicitação: {self.motivo_atraso_entry.get("1.0", END)}'
            print("Dados a serem copiados:\n" + dados)
        elif solicitacao == "Extensao Retorno":
            # teste = self.custom_text_for("Extensao Retorno", True)
            # print('teste:\n' + teste + '\n' + '-'*10)
            request_dados = self.custom_text_for(solicitacao, True)
            print(request_dados)
            if request_dados == None:
                msg = False
            else:
                msg = True
            try:
        ...
        # adicionar lógica para mensagem personalizada
            ...
            return print("aconteceu alguma coisa")
        ...
        try:
            ...
            if solicitacao == "Padrão":
                ...
                self.dados_solicitacao = [
                    ...
                    ("Procedimento", self.procedimento_entry.get()),
                    ("Profissional", self.profissional_entry.get()),
                    ("Motivo do pedido de extensão do retorno:", str(self.custom_text_for(solicitacao, tratado=True)))]
                print(self.dados_solicitacao)
    ...
    def criar_botoes(self):
        """Criando botões"""
        botoes_de_funcao = True
        if botoes_de_funcao == True:
            try:
    ...
    def wpp_link(self):
        ...
        text = str(self.copy_dados(msg=False, to_whatsapp=True))
...
,
    ...
    def atualizar_paciente(self, event):
        ...
        :type event: object
        """
        self.nome_paciente_label.config(text=self.paciente_entry.get())  # Atualiza o valor da label com o conteúdo da entry

    def atualizar_procediemento(self, event):
        """exemplo de uso  variavel.bind("<KeyRelease>", self.atualizar_label)
        :type event: object
        """
